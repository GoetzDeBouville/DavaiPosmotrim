...
class MatchedSessionViewModel @Inject constructor(
    ...
    // После подключения data слоя убрать Suppress
    @Suppress("Detekt.UnusedParameter")
    fun getSessionData(sessionId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            _state.emit(MatchedSessionState.Loading)

            getData.getData().fold(
                onSuccess = { movies ->
                    _state.value =
                        if (movies.isEmpty()) {
                            MatchedSessionState.Empty
                        } else {
                            MatchedSessionState.Data(
                                data = sessionList
                            )
                        }
                },
                onError = { errorType ->
                    _state.value = MatchedSessionState.Error(errorType)
                }
            )
        }
    }
...
private val sessionList = SessionWithMovies(
    session = Session(
        id = "minim",
        users = listOf("Саша", "Маша", "Стёпа"),
        numberOfMatchedMovies = 4,
        date = 1720708603000,
        status = SessionStatus.VOTING,
        imgUrl = "http://www.bing.com/search?q=patrioque"
    ), movies = listOf(
        MovieDetails(
            id = 12345,
            name = "Интерстеллар",
            ...
            year = "2014",
            countries = listOf("США", "Канада", "Португалия"),
            ...
            year = "2008",
            countries = listOf("США", "Китай", "Канада"),
